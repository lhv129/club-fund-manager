...
export function ExchangeSessionPlayerPaymentsPageClient() {
    ...
    const columns: ColumnDef<ExchangeSessionPlayer>[] = [
        { key: "session", label: x("session"), render: (row) => row.exchange_session ? <div><p className="whitespace-nowrap text-sm font-medium">{row.exchange_session.session_date} · {row.exchange_session.court_name}</p><p className="text-xs text-foreground-muted">#{row.exchange_session.id}</p></div> : `#${row.exchange_session_id}` },
        ...
        { key: "paid", label: x("paid"), render: (row) => <Badge variant={row.paid ? "active" : "pending"} title={row.paid ? x("paidYes") : x("paidNo")} /> },
        { key: "created_at", label: t("createdAt"), render: (row) => row.created_at ? formatDateTime(row.created_at, locale) : "—" },
    ];
    ...
    return <div className="space-y-6">
        ...
        />
        <DataTable table={{ columns, data: players.data, loading: players.isLoading, fetching: players.isFetching, keyExtractor: (row) => row.id, emptyText: x("noPlayers") }} pagination={{ page: params.page, limit: params.limit, total: players.total, onPageChange: setPage, onLimitChange: setLimit }} />
    </div>;
}
