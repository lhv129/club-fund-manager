...
export function TransactionsPageClient() {
    ...
    return <div className="space-y-6">
        ...
        <FilterBar search={params.search} isActive={params.is_active} sortBy={params.sort_by} sortDir={params.sort_dir} sortOptions={[{ value: "transaction_date", label: tt("transactionDate") }, { value: "amount", label: tt("amount") }, { value: "balance", label: tt("balance") }, { value: "type", label: tt("type") }, { value: "sort_order", label: t("sortOrder") }, { value: "created_at", label: t("createdAt") }]} loading={transactions.isLoading} extraFilters={extraFilters} onApply={(filters: AppliedFilters) => updateMany({ ...filters, bank_account_id: draftBank, type: draftType, from_date: draftFrom || undefined, to_date: draftTo || undefined })} onReset={() => { setDraftBank(undefined); setDraftType(undefined); setDraftFrom(""); setDraftTo(""); reset(); }} />
        <Table columns={columns} data={transactions.data} loading={transactions.isLoading} keyExtractor={(row) => row.id} emptyText={tt("notFound")} renderActions={(row) => <TableActions><TableActionItem icon={<Eye className="h-4 w-4" />} label={t("view")} onClick={() => router.push(`${clubRoute(slug, CLUB_SUBROUTES.transactions)}/${row.id}` as never)} />{canUpdate && <TableActionItem icon={<Pencil className="h-4 w-4" />} label={t("edit")} onClick={() => { setSelected(row); setModalOpen(true); }} />}{canDelete && <TableActionItem icon={<Trash2 className="h-4 w-4" />} label={t("delete")} variant="danger" onClick={() => setDeleteTarget(row)} />}</TableActions>} />
        <Pagination page={params.page} limit={params.limit} total={transactions.total} onPageChange={setPage} onLimitChange={setLimit} />
