...
export function PaymentCodesPageClient() {
    ...
    return <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-2.5 rounded-2xl border border-border bg-background px-3 py-3.5 shadow-sm sm:px-4">
            <label className="flex w-full min-w-0 flex-1 flex-col gap-1 sm:min-w-56"><span className="text-xs font-medium text-foreground-muted">{tp("contributionId")}</span><input type="number" min={1} value={contributionId} onChange={(e) => setContributionId(e.target.value)} placeholder={tp("contributionIdPlaceholder")} className="h-10 rounded-xl border border-border bg-background px-3 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40" /></label>
            <div className="flex flex-col gap-1"><span className="text-xs font-medium text-foreground-muted">{t("status")}</span><Select label={t("status")} value={status ?? ""} onChange={(value) => setStatus((value || undefined) as PaymentCodeStatus | undefined)} placeholder={t("all")} options={[{ value: "pending", label: tp("pending") }, { value: "used", label: tp("used") }, { value: "expired", label: tp("expired") }]} /></div>
            <button type="button" onClick={() => { setContributionId(""); setStatus(undefined); reset(); }} className="flex h-10 items-center gap-1.5 rounded-xl border border-border px-3 text-sm font-medium text-foreground-muted hover:bg-background-subtle"><RotateCcw className="h-3.5 w-3.5" />{t("reset")}</button>
            <button type="button" disabled={query.isFetching} onClick={() => updateMany({ monthly_contribution_id: contributionId ? Number(contributionId) : undefined, status })} className="flex h-10 items-center gap-1.5 rounded-xl bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary-hover disabled:opacity-60"><Search className="h-4 w-4" />{t("searchButton")}</button>
        </div>
