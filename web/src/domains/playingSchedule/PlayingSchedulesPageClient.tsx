...
export function PlayingSchedulesPageClient() {
    ...
    return (
        <div className="space-y-6">
            ...
            <FilterBar searchClassName="basis-full sm:w-auto sm:basis-auto" search={params.search} sortBy={params.sort_by} sortDir={params.sort_dir} sortOptions={[{ value: "weekday", label: tp("weekday") }, { value: "sort_order", label: t("sortOrder") }, { value: "created_at", label: t("createdAt") }]} showStatusFilter={false} loading={schedules.isFetching} onApply={(filters: AppliedFilters) => updateMany({ search: filters.search, sort_by: filters.sort_by, sort_dir: filters.sort_dir, weekday: draftWeekday, is_active: draftActive })} onReset={() => { setDraftWeekday(undefined); setDraftActive(undefined); reset(); }} extraFilters={<><div className="flex w-full flex-col gap-1 sm:w-auto"><span className="text-xs font-medium text-foreground-muted">{tp("weekday")}</span><Select className="w-full sm:w-auto" label={tp("weekday")} options={WEEKDAYS.map((value) => ({ value: String(value), label: tp(`weekday_${value}`) }))} value={draftWeekday === undefined ? "" : String(draftWeekday)} onChange={(value) => setDraftWeekday(value === "" ? undefined : Number(value))} placeholder={t("all")} /></div><div className="flex w-full flex-col gap-1 sm:w-auto"><span className="text-xs font-medium text-foreground-muted">{t("status")}</span><Select className="w-full sm:w-auto" label={t("status")} options={[{ value: "1", label: t("active") }, { value: "0", label: t("inactive") }]} value={draftActive === undefined ? "" : String(draftActive)} onChange={(value) => setDraftActive(value === "" ? undefined : Number(value) as 0 | 1)} placeholder={t("all")} /></div></>} />
            <DataTable table={{ columns, data: schedules.data, loading: schedules.isLoading, fetching: schedules.isFetching, keyExtractor: (row) => row.id, emptyText: tp("notFound"), renderActions: (row) => <TableActions>{canUpdate && <TableActionItem icon={<Pencil className="h-4 w-4" />} label={t("edit")} onClick={() => { setSelected(row); setModalOpen(true); }} />}{canDelete && <TableActionItem icon={<Trash2 className="h-4 w-4" />} label={t("delete")} variant="danger" onClick={() => setDeleteTarget(row)} />}</TableActions> }} pagination={{ page: params.page, limit: params.limit, total: schedules.total, onPageChange: setPage, onLimitChange: setLimit }} />
