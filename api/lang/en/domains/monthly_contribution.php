<?php

return [
    'list' => 'Monthly contribution list.',
    'select' => 'Monthly contribution list (dropdown).',
    'detail' => 'Monthly contribution details.',
    'created' => 'Monthly contribution created successfully.',
    'updated' => 'Monthly contribution updated successfully.',
    'deleted' => 'Monthly contribution deleted successfully.',
    'not_found' => 'Monthly contribution not found.',
    'status_toggled' => 'Monthly contribution status updated successfully.',
    'invalid_gender' => 'Invalid user gender.',
    'already_exists' => 'This member has already made a contribution for this period.',
    'cancelled_bank_exists' => 'The cancelled bank contribution still has an audit transaction. Update the existing record instead of creating a new one.',
    'payment_method_required' => 'Please select a payment method.',
    'transaction_required' => 'A paid monthly contribution must be linked to a transaction.',
    'invalid_payment_transaction' => 'The transaction does not match the club or selected payment method.',
'cash_transaction_description' => 'Cash payment for monthly contribution :month/:year - :fullname',

    'attributes' => [
        'club_id' => 'Club',
        'user_id' => 'Member',
        'period_id' => 'Fund period',
        'transaction_id' => 'Transaction',
        'amount' => 'Amount',
        'status' => 'Status',
        'paid_by' => 'Payment method',
        'payment_date' => 'Payment date',
        'is_active' => 'Active',
    ],
];
